...
import { render, rebuildBoard } from './renderer.js';
...
function _finishLoad({ label, selectValue, expertNote = '', hint = '', gridCols = 4 }) {
  ...
  setMsg(''); updateBestDisplay(); stopConfetti(); mascotIdle();
  rebuildBoard(gridCols);
  render();
  updateRestartBtn(); updateSOSBtn();
}
...
export function loadGravity() {
  ...
  _finishLoad({ label: 'Gravity', selectValue: 5, hint: 'gravity flips every 4 moves', gridCols: 5 });
}
